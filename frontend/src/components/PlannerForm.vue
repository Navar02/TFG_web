        ...
        <v-card class="pa-4">
          ...
          <div class="search-container">
            ...
            <transition name="slide-fade">
              <v-list
                v-if="citySuggestions.length && isCityListVisible"
          ...
          <v-select
            ...
            :items="categories"
            label="Categorías de los lugares a recomendar"
  ...
  methods: {
    ...
    async submitForm() {
      ...
      if (missingFields.length > 0) {
        alert(
          `Por favor, rellena los siguientes campos: ${missingFields.join(
            ", "
          )}`
        );
        return;
    ...
    handleStartDateChange() {
      ...
      if (
        startDateParts[0] < todayParts[0] ||
        (startDateParts[0] === todayParts[0] &&
          startDateParts[1] < todayParts[1]) ||
        (startDateParts[0] === todayParts[0] &&
          startDateParts[1] === todayParts[1] &&
          startDateParts[2] < todayParts[2])
      ) {
    ...
    validateEndDate() {
      ...
      if (
        endDateParts[0] < startDateParts[0] ||
        (endDateParts[0] === startDateParts[0] &&
          endDateParts[1] < startDateParts[1]) ||
        (endDateParts[0] === startDateParts[0] &&
          endDateParts[1] === startDateParts[1] &&
          endDateParts[2] < startDateParts[2])
      ) {
